...
public class Main extends javax.swing.JFrame {
     ...
     */
    public Main() {
        this.screen1 = new TelaPrincipal();
    ...
    private void initComponents() {
        ...
        jFormattedTextFieldIdentificador.setFormatterFactory(new javax.swing.text.DefaultFormatterFactory(new javax.swing.text.NumberFormatter(new java.text.DecimalFormat("######"))));
        jFormattedTextFieldIdentificador.setHorizontalAlignment(javax.swing.JTextField.CENTER);
        jFormattedTextFieldIdentificador.setText("Identificador");
        ...
        try {
            switch (op) {
                ...
                case 2:
                        ...
                        try{
                            if(aux.equals("") == false){
                                preparedStatement = con.prepareStatement("SELECT id , materia FROM produtos");
                                rs = (ResultSet) preparedStatement.executeQuery();  
                                while(rs.next()){
                                    String materiat="";
                                    String reply[] = rs.getString("materia").split("@");
                                    
                                    for (int i = 0; i < reply.length; i++) {
                                       String out[] = reply[i].split("#");
                                       if(out[1].equalsIgnoreCase(aux)){
                                           out[1]=codigo;
                                       }
                                       reply[i] = "";
                                    ...
                                    System.out.println(materiat);
                                     int idm = Integer.parseInt(rs.getString("id"));
                                        preparedStatement = con.prepareStatement("UPDATE produtos SET materia = ?  WHERE id = "+idm);
                                        preparedStatement.setString(1, materiat);
                                        preparedStatement.executeUpdate();  
                                }//end while
    ...
    public  synchronized static Pao atualizaMysqlProduto(Pao dados, int op) throws ClassNotFoundException, SQLException{
         ...
         System.out.println("Tentando conectar ao MySQL via Atualiza MYSQL...");
           ...
           Class.forName("com.mysql.jdbc.Driver");
            ...
            String valor = dados.getValor();
            String materia = dados.getMateria_prima();
        ...
        try {
            switch (op) {
                case 1:
                    ...
                    break;
                case 2:
                        preparedStatement = con.prepareStatement("UPDATE produtos SET codigo = ?, tipo = ?, valor = ?, validade = ?, peso = ?, materia =?  WHERE id = "+id);
                        preparedStatement.setString(1, codigo);
                        preparedStatement.setString(2, tipo);
                        preparedStatement.setString(3, valor);
                        preparedStatement.setString(4, validade);
                        ...
                        try{ /* Verifica se já existe um produtos registro inserido */     
                            ...
                            if(codigo.equals("") == false){
                                ...
                                if(existe){
                                    ...
                                    dados.setMateria_prima(rs.getString("materia"));
                                } else {
                                     dados.setTipo_pao("");
                                    dados.setCodigo("");
                                    dados.setValor("");
